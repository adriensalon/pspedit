...
void import_stbimage(const std::filesystem::path& stbimage_path, editor_project& project)
{
    ...
    image_object _image = project.images.default_object ? project.images.default_object.value() : image_object(); // TODO HANDLE DEFAULTS
    ...
    _image.stride_width = static_cast<u32>(_source_width) * _bytes_per_pixel;
    _image.format = texture_internal_format::rgba8888;
    _image.min_filter = texture_filter::nearest;
    _image.mag_filter = texture_filter::nearest;
    _image.swizzle = 0;
    _image.width = static_cast<u16>(_source_width);
    _image.height = static_cast<u16>(_source_height);
