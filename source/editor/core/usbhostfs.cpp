// original code written 2006 James F <tyranid@gmail.com>
...
// packed structs on all compilers
#if defined(_MSC_VER)
#define PACKED_STRUCT(def) __pragma(pack(push, 1)) def __pragma(pack(pop))
#elif defined(__GNUC__)
#define PACKED_STRUCT(def) def __attribute__((packed))
#else
#define PACKED_STRUCT(def) def
#endif
...
#define rmdir _rmdir
inline int psp_mkdir(const char* p, int) { return _mkdir(p); }
#define mkdir psp_mkdir
#define chmod _chmod
...
inline void sleep(unsigned int sec) { ::Sleep(sec * 1000u); }
...
static DirHandle open_dirs[MAX_DIRS];

static libusb_context* usbctx = NULL;
static libusb_device_handle* usbhdr = NULL;
...
int find_nocase(const char* rootdir, const char* relpath, char* token)
{
    ...
    char abspath[PATH_MAX];
    char match[PATH_MAX];
    int len = snprintf(abspath, PATH_MAX, "%s%s", rootdir, relpath);
    ...
    for (auto const& entry : fs::directory_iterator(p, ec)) {
        if (ec)
            break;
        auto name = entry.path().filename().u8string();

        if (strcasecmp(name.c_str(), token) == 0) {
            int tmp = calc_rating(token, name.c_str());
            if (tmp > rating) {
                strncpy(match, name.c_str(), PATH_MAX - 1);
                match[PATH_MAX - 1] = 0;
                rating = tmp;
            }
            found = true;
...
int make_path(unsigned int drive, const char* path, char* retpath, int dir)
{
    ...
    std::lock_guard<std::mutex> lock(g_drivemtx);
...
int dir_open(int drive, const char* dirname)
{
    ...
    std::sort(names.begin(), names.end(), [](const fs::path& a, const fs::path& b) {
        auto sa = a.u8string();
        auto sb = b.u8string();
        return sa < sb;
    });

    int dirnum = (int)names.size();
    if (dirnum <= 0)
        return GETERROR(ENOENT);

    memset(&open_dirs[did], 0, sizeof(open_dirs[did]));
    open_dirs[did].pDir = (SceIoDirent*)malloc(sizeof(SceIoDirent) * dirnum);
    if (!open_dirs[did].pDir)
        return GETERROR(ENOMEM);

    memset(open_dirs[did].pDir, 0, sizeof(SceIoDirent) * dirnum);

    for (int i = 0; i < dirnum; i++) {
        auto s = names[i].u8string();
        strncpy(open_dirs[did].pDir[i].name, s.c_str(), sizeof(open_dirs[did].pDir[i].name) - 1);
        open_dirs[did].pDir[i].name[sizeof(open_dirs[did].pDir[i].name) - 1] = 0;

        if (fill_stat(fulldir, open_dirs[did].pDir[i].name, &open_dirs[did].pDir[i].stat) < 0) {
...
int handle_hello(libusb_device_handle* dev)
{
    ...
    return euid_usb_bulk_write(dev, 0x2, (char*)&resp, sizeof(resp), 10000);
}

int handle_open(libusb_device_handle* dev, struct HostFsOpenCmd* cmd, int cmdlen)
{
    ...
    do {
        ...
        ret = euid_usb_bulk_write(dev, 0x2, (char*)&resp, sizeof(resp), 10000);
    } while (0);

    return ret;
}

int handle_dopen(libusb_device_handle* dev, struct HostFsDopenCmd* cmd, int cmdlen)
{
...
int fixed_write(int fd, const void* data, int len)
{
    ...
    while (byteswrite < len) {
        ...
        ret = write(fd, p + byteswrite, len - byteswrite);
        if (ret < 0) {
            if (errno != EINTR) {
...
int handle_write(libusb_device_handle* dev, struct HostFsWriteCmd* cmd, int cmdlen)
{
...
int fixed_read(int fd, void* data, int len)
{
    ...
    while (bytesread < len) {
        ...
        ret = read(fd, (void*)(p + bytesread), len - bytesread);
        if (ret < 0) {
            if (errno != EINTR) {
...
int handle_read(libusb_device_handle* dev, struct HostFsReadCmd* cmd, int cmdlen)
{
    ...
    do {
        ...
        if (LE32(resp.cmd.extralen) > 0) {
            ret = euid_usb_bulk_write(dev, 0x2, read_block, LE32(resp.cmd.extralen), 10000);
        }
    } while (0);
...
int handle_close(libusb_device_handle* dev, struct HostFsCloseCmd* cmd, int cmdlen)
{
...
int handle_dclose(libusb_device_handle* dev, struct HostFsDcloseCmd* cmd, int cmdlen)
{
...
int handle_dread(libusb_device_handle* dev, struct HostFsDreadCmd* cmd, int cmdlen)
{
    ...
    do {
        ...
        if (LE32(resp.cmd.extralen) > 0) {
            ret = euid_usb_bulk_write(dev, 0x2, (char*)dir, LE32(resp.cmd.extralen), 10000);
        }
    } while (0);
...
int handle_lseek(libusb_device_handle* dev, struct HostFsLseekCmd* cmd, int cmdlen)
{
...
int handle_remove(libusb_device_handle* dev, struct HostFsRemoveCmd* cmd, int cmdlen)
{
...
int handle_rmdir(libusb_device_handle* dev, struct HostFsRmdirCmd* cmd, int cmdlen)
{
...
int handle_mkdir(libusb_device_handle* dev, struct HostFsMkdirCmd* cmd, int cmdlen)
{
...
int handle_getstat(libusb_device_handle* dev, struct HostFsGetstatCmd* cmd, int cmdlen)
{
    ...
    do {
        ...
        if (LE32(resp.cmd.extralen) > 0) {
            ret = euid_usb_bulk_write(dev, 0x2, (char*)&st, sizeof(st), 10000);
        }
    } while (0);
...
int handle_chstat(libusb_device_handle* dev, struct HostFsChstatCmd* cmd, int cmdlen)
{
...
int handle_rename(libusb_device_handle* dev, struct HostFsRenameCmd* cmd, int cmdlen)
{
...
int handle_chdir(libusb_device_handle* dev, struct HostFsChdirCmd* cmd, int cmdlen)
{
...
int handle_ioctl(libusb_device_handle* dev, struct HostFsIoctlCmd* cmd, int cmdlen)
{
    ...
    do {
        ...
        if (inlen > 0) {
            ...
            ret = euid_usb_bulk_read(dev, 0x81, inbuf, inlen, 10000);
            if (ret != inlen) {
        ...
        if (LE32(resp.cmd.extralen) > 0) {
            ret = euid_usb_bulk_write(dev, 0x2, (char*)outbuf, LE32(resp.cmd.extralen), 10000);
        }
    } while (0);
...
int get_drive_info(struct DevctlGetInfo* info, unsigned int drive)
{
    ...
    std::error_code ec;
    fs::path p = fs::u8path(g_drives[drive].rootdir);

    // filesystem::space wants an existing path; rootdir should be valid
    auto sp = fs::space(p, ec);
    if (ec) {
        fprintf(stderr, "Could not stat %s (%s)\n", g_drives[drive].rootdir, ec.message().c_str());
        return -1;
    }

    // PSP side expects blocks; historically you used 512-byte blocks.
    constexpr uint64_t kBlock = 512;

    info->btotal = (uint32_t)(sp.capacity / kBlock);
    info->bfree = (uint32_t)(sp.available / kBlock);
    info->unk = info->btotal;
    info->ssize = 512;

    // Try to compute "sects" similarly to old code (blocksize/512).
    // Use GetDiskFreeSpaceW for sectors/cluster and bytes/sector if possible.
    DWORD spc = 0, bps = 0, nfc = 0, tc = 0;

    std::wstring wpath = p.wstring();
    // GetDiskFreeSpace requires a root path like "C:\"
    // If rootdir is deeper, ask for the volume root.
    wchar_t volumeRoot[MAX_PATH] = {};
    if (GetVolumePathNameW(wpath.c_str(), volumeRoot, MAX_PATH) && GetDiskFreeSpaceW(volumeRoot, &spc, &bps, &nfc, &tc)) {
        // cluster size in bytes = spc * bps
        uint64_t clusterBytes = (uint64_t)spc * (uint64_t)bps;
        info->sects = (uint32_t)(clusterBytes / 512);
    } else {
        // Fallback: 1 sector of 512 bytes
        info->sects = 1;
    }
...
int handle_devctl(libusb_device_handle* dev, struct HostFsDevctlCmd* cmd, int cmdlen)
{
    static char inbuf[64 * 1024];
    static char outbuf[64 * 1024];
    int inlen;
...
libusb_device_handle* wait_for_device(void)
{
    ...
    while (!usbdev) {
        std::size_t devcnt = libusb_get_device_list(usbctx, &devs);

        for (std::size_t i = 0; i < devcnt; i++) {
...
void do_hostfs(struct HostFsCmd* cmd, int readlen)
{
    ...
    case HOSTFS_CMD_HELLO:
        if (handle_hello(usbhdr) < 0) {
            fprintf(stderr, "Error sending hello response\n");
        }
        break;
    case HOSTFS_CMD_OPEN:
        if (handle_open(usbhdr, (struct HostFsOpenCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in open command\n");
        }
        break;
    case HOSTFS_CMD_CLOSE:
        if (handle_close(usbhdr, (struct HostFsCloseCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in close command\n");
        }
        break;
    case HOSTFS_CMD_WRITE:
        if (handle_write(usbhdr, (struct HostFsWriteCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in write command\n");
        }
        break;
    case HOSTFS_CMD_READ:
        if (handle_read(usbhdr, (struct HostFsReadCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in read command\n");
        }
        break;
    case HOSTFS_CMD_LSEEK:
        if (handle_lseek(usbhdr, (struct HostFsLseekCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in lseek command\n");
        }
        break;
    case HOSTFS_CMD_DOPEN:
        if (handle_dopen(usbhdr, (struct HostFsDopenCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in dopen command\n");
        }
        break;
    case HOSTFS_CMD_DCLOSE:
        if (handle_dclose(usbhdr, (struct HostFsDcloseCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in dclose command\n");
        }
        break;
    case HOSTFS_CMD_DREAD:
        if (handle_dread(usbhdr, (struct HostFsDreadCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in dread command\n");
        }
        break;
    case HOSTFS_CMD_REMOVE:
        if (handle_remove(usbhdr, (struct HostFsRemoveCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in remove command\n");
        }
        break;
    case HOSTFS_CMD_RMDIR:
        if (handle_rmdir(usbhdr, (struct HostFsRmdirCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in rmdir command\n");
        }
        break;
    case HOSTFS_CMD_MKDIR:
        if (handle_mkdir(usbhdr, (struct HostFsMkdirCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in mkdir command\n");
        }
        break;
    case HOSTFS_CMD_CHDIR:
        if (handle_chdir(usbhdr, (struct HostFsChdirCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in chdir command\n");
        }
        break;
    case HOSTFS_CMD_RENAME:
        if (handle_rename(usbhdr, (struct HostFsRenameCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in rename command\n");
        }
        break;
    case HOSTFS_CMD_GETSTAT:
        if (handle_getstat(usbhdr, (struct HostFsGetstatCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in getstat command\n");
        }
        break;
    case HOSTFS_CMD_CHSTAT:
        if (handle_chstat(usbhdr, (struct HostFsChstatCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in chstat command\n");
        }
        break;
    case HOSTFS_CMD_IOCTL:
        if (handle_ioctl(usbhdr, (struct HostFsIoctlCmd*)cmd, readlen) < 0) {
            fprintf(stderr, "Error in ioctl command\n");
        }
        break;
    case HOSTFS_CMD_DEVCTL:
        if (handle_devctl(usbhdr, (struct HostFsDevctlCmd*)cmd, readlen) < 0) {
                ...
                if (g_servsocks[i] >= 0) {
                    if (FD_ISSET(g_servsocks[i], &read_set)) {
                        ...
                        if (g_clientsocks[i] >= 0) {
                            ...
                            setsockopt(g_clientsocks[i], IPPROTO_TCP, TCP_NODELAY, (const char*)&flag, sizeof(flag));
...
bool usbhostfs_start(const usbhostfs_options& options)
{
    return false;
}