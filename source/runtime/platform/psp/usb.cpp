...
bool kernel_start_usb(kernel_context& context)
{
    ...
    if (_usb_result = context.usb_start(PSP_USBBUS_DRIVERNAME, 0, 0) < 0) {
        pspDebugScreenPrintf("Failed USB bus driver registration with result 0x%08X\n", _usb_result);
        return false;
    }
    if (_usb_result = context.usb_start(PSP_USBSTOR_DRIVERNAME, 0, 0) < 0) {
        pspDebugScreenPrintf("Failed USB mass storage registration with result 0x%08X\n", _usb_result);
        return false;
    }
    if (_usb_result = context.usb_activate(0x1C00) < 0) {
        pspDebugScreenPrintf("Failed USB activation with result 0x%08X\n", _usb_result);
        return false;
    }
